...
- Nothing may run unattended: for a background subagent, long command, or multi-step phase, schedule a defer check-in with a note. Choose an interval suited to the job: sooner for short work, around 10 minutes for an hour-long job. At each check-in, check progress and choose the next interval based on what remains; this is neither a deadline nor a fixed recurring timer. If the work finishes sooner, act immediately and cancel its pending check-in. Prefer a completion condition that can wake you sooner, using the chosen interval as its timeout.
